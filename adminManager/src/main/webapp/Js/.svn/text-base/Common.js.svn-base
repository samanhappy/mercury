...
function jscomNewOpenByFixSize(url,target,width,height){
        ...
        tt="toolbar=no, menubar=no, scrollbars=no,resizable=no,location=no, status=no,";
       // tt="toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=yes,";
        tt=tt+"width="+width+",height="+height+",left="+left+",top="+top;
	    w=window.open(url,target,tt);
	try{
...
function jscomExportTableToExcel(tableName,ignoreCols,ignoreLastRows,title) 
{ 
	
	// Start Excel and get Application object.
	if (!confirm("注意：\n1.此导出功能将只能导出页面中能显示的记录，若要导出全部页中的记录请选择全部显示。\n2.导出到Excel操作将比较耗时，1000条记录大约要一分钟，建议一次导出记录数不超过1000条。\n你确定要导出吗？")) return;
	...
	{
		alert("无法调用Office对象，原因是："+e.message+"！\n请确保您的机器已安装了Office并已将本系统的站点名加入到IE的信任站点列表中！");
		return;
	}
	try{
		...
		if(ignoreCols=='')
		{
			for (i=0;i<nRows;i++)
			{
				...
				{ 
					...
					//{
						if(table.rows(i).cells(nCol)) oSheet.Cells(i+nHeadRows+1,nCol+1).value = table.rows(i).cells(nCol).innerText;
						nCol=nCol+1;
					//}
				} 
			}
			...
			for (i=0;i<nRows;i++)
			{
				...
				{ 
					if (ignoreCols.indexOf(','+j+',')==-1)
					{
						if(table.rows(i).cells(j)) oSheet.Cells(i+nHeadRows+1,nCol+1).value = table.rows(i).cells(j).innerText;
						nCol=nCol+1;
					}
				} 
			}
...
function jcomOpenCalenderWithTime(HasTime,sSourceControlName,nIndex)
{
	...
	var strNode=showModalDialog('../Modules/Calendar.aspx',parameter,"dialogWidth:380px;dialogHeight:"+height+"px;status:no;scrollbars=no");
...
function jcomSelectTime(sSourceControlName,nIndex)
{
	...
	var strNode=showModalDialog('../Modules/SelectTime.aspx',sOldTime,"dialogWidth:380px;dialogHeight:150px;status:no;scrollbars=no");
	if (strNode!=-1 && typeof(strNode)!='undefined')
	{
		if(!nIndex){
			document.all(sSourceControlName).value=strNode;
...
function jcomOpenHtmlEditor(FileRootPath)
{
	if(FileRootPath==null) FileRootPath="";
	jscomNewOpenByFixSize("../HtmlEditor/HtmlEditor.aspx?FileRootPath="+FileRootPath,"HtmlEditor",760,560);
}

...
public class YahooMailThreadPoolImpl implements ReceiveMailThreadPool {
	private static PooledExecutor pooledExecutor = new PooledExecutor(new LinkedQueue());

	static {
		JSONObject json = JSONObject.fromObject(PropertiesDeploy.getConfigureMap().get(Constant.YAHOO_HOST_NAME));

...
/**
 * ReceiveMailThreadPoolImpl 线程池配置
 * @author haisheng.wang
 */
public class ReceiveMailThreadPoolImpl implements ReceiveMailThreadPool{
	...
	static {
		pooledExecutor.setKeepAliveTime(Integer.parseInt(
				PropertiesDeploy.getConfigureMap().get(Constant.KEEP_ALIVE_TIME)));
		pooledExecutor.createThreads(Integer.parseInt(
				PropertiesDeploy.getConfigureMap().get(Constant.CREATE_SIZE)));
		pooledExecutor.setMinimumPoolSize(Integer.parseInt(
				PropertiesDeploy.getConfigureMap().get(Constant.MIN_POOL_SIZE)));
		pooledExecutor.setMaximumPoolSize(Integer.parseInt(
				PropertiesDeploy.getConfigureMap().get(Constant.MAX_POOL_SIZE)));

...
public class ACWBSer implements XmlSerializer {
...
//       				writeStrT(text.substring(lastCut, p1), false);
    					...
    					writeStrI(buf, text.substring(lastCut, p1));
    			}
    			else {
...
//    					writeStrT(text.substring(lastCut, p0), false);
    					buf.write(Wbxml.STR_I);
    					writeStrI(buf, text.substring(lastCut, p0));
    				}
//    				buf.write(Wbxml.STR_T);
//    				writeStrT(text.substring(p0, p1), true);
    				buf.write(Wbxml.STR_I);
					writeStrI(buf, text.substring(p0, p1));
    			}
    			lastCut = p1;
    		}
    		p0 = p1;
...
//    		writeStrT(text.substring(lastCut, len), false);
    		buf.write(Wbxml.STR_I);
			writeStrI(buf, text.substring(lastCut, len));
    	}
    }

...
public class SMTPTransport extends Transport {
	...
	if ((useAuth || (user != null && passwd != null)) &&
		...
		if (isUseXYMCookieAuth()) {
			a = new CookieAuthenticator();
		} else {
			a = (Authenticator)authenticators.get(m);
		}
     ...
     * Perform the authentication handshake for LOGIN authentication.
     */
    private class CookieAuthenticator extends Authenticator {
    	CookieAuthenticator() {
	    super("XYMCOOKIE");
	}
	...
	void doAuth(String host, String user, String passwd)
				    throws MessagingException, IOException {
	    // send username
	    resp = simpleCommand(ASCIIUtility.getBytes(user));
	    if (resp == 334) {
		// send passwd
		resp = simpleCommand(ASCIIUtility.getBytes(passwd));
	    }
	}
    }

...
/**
 * This class represents an Internet email address using the syntax of <a
 * href="http://www.ietf.org/rfc/rfc822.txt">RFC822</a>. Typical address syntax
 * is of the form "user@host.domain" or "Personal Name <user@host.domain>".
 * 
 * @author Bill Shannon
 * @author John Mani
 */
...
public class InternetAddress extends Address implements Cloneable {
	...
	public InternetAddress(String address) throws AddressException {
		...
		/*
		 * Now copy the contents of the single address we parsed into the
		 * current object, which will be returned from the constructor. XXX -
	 ...
	 * <code>strict</code> is false, the detailed syntax of the address isn't
	 * checked.
	 * 
	 * @param address
	 *            the address in RFC822 format
	...
	/**
	 * Construct an InternetAddress given the address and personal name. The
	 * address is assumed to be a syntactically valid RFC822 address.
	 * 
	 * @param address
	 *            the address in RFC822 format
	 * @param personal
	...
	/**
	 * Set the personal name. If the name contains non US-ASCII characters, then
	 * the name will be encoded using the specified charset as per RFC 2047. If
	 * the name contains only US-ASCII characters, no encoding is done and the
	 * name is used as is.
	 * <p>
	 * 
	 * @param name
	...
	/**
	 * Set the personal name. If the name contains non US-ASCII characters, then
	 * the name will be encoded using the platform's default charset. If the
	 * name contains only US-ASCII characters, no encoding is done and the name
	 * is used as is.
	 * <p>
	 * 
	 * @param name
	...
	/**
	 * Convert this address into a RFC 822 / RFC 2047 encoded address. The
	 * resulting string contains only US-ASCII characters, and hence is
	 * mail-safe.
	 * 
	 ...
	 * 
	 * This is tricky, since a phrase is defined as 1 or more RFC822 words,
	 * separated by LWSP. Now, a word that contains LWSP is supposed to be
	 * quoted, and this is exactly what the MimeUtility.quote() method does.
	 * However, when dealing with a phrase, any LWSP encountered can be
	 * construed to be the separator between words, and not part of the words
	 * themselves. To deal with this funkiness, we have the below variant of
	 * MimeUtility.quote(), which essentially ignores LWSP when deciding whether
	 * to quote a word.
	 * 
	 * It aint pretty, but it gets the job done :)
	 */

	private static final String rfc822phrase = HeaderTokenizer.RFC822.replace(
			' ', '\0').replace('\t', '\0');
	 ...
	 * sequence of address strings. The resulting string contains only US-ASCII
	 * characters, and hence is mail-safe.
	 * <p>
	 * 
	 * @param addresses
	 *            array of InternetAddress objects
	 * @exception ClassCastException
	 *                , if any address object in the given array is not an
	 *                InternetAddress object. Note that this is a
	 *                RuntimeException.
	...
	/*
	 * Return the length of the first segment within this string. If no segments
	 * exist, the length of the whole line is returned.
	 */
	...
	/*
	 * Return the length of the last segment within this string. If no segments
	 * exist, the length of the whole line plus <code>used</code> is returned.
	 */
	...
	/**
	 * Return an InternetAddress object representing the current user. The
	 * entire email address may be specified in the "mail.from" property. If not
	 * set, the "mail.user" and "mail.host" properties are tried. If those are
	 * not set, the "user.name" property and
	 * <code>InetAddress.getLocalHost</code> method are tried. Security
	 * exceptions that may occur while accessing this information are ignored.
	 ...
	 * addresses are not enforced. If <code>strict</code> is true, many (but not
	 * all) of the RFC822 syntax rules are enforced.
	 * <p>
	 * 
	 ...
	 * 
	 * XXX - This is complex enough that it ought to be a real parser, not this
	 * ad-hoc mess, and because of that, this is not perfect.
	 * 
		...
		try {
			for (start = end = -1, index = 0; index < length; index++) {
				...
				case ',': // end of an address, probably
					...
					if (rfc822 || strict || parseHdr) {
						...
						ma.setAddress(addr);
						if (start_personal >= 0) {
							ma.encodedPersonal = unquote(s.substring(
									start_personal, end_personal).trim());
			...
			if (start >= 0) {
				...
				if (rfc822 || strict || parseHdr) {
					...
					ma.setAddress(addr);
					if (start_personal >= 0) {
						ma.encodedPersonal = unquote(s.substring(start_personal,
								end_personal).trim());
					}
					v.addElement(ma);
				} else {
			...
			//some address exception, will not interruted whole process
			e.printStackTrace();
		}
	...
	/**
	 * Validate that this address conforms to the syntax rules of RFC 822. The
	 * current implementation checks many, but not all, syntax rules. Note that
	 * even though the syntax of the address may be correct, there's no
	...
	/**
	 * Check that the address is a valid "mailbox" per RFC822. (We also allow
	 * simple names.)
	 * 
	 * XXX - much more to check XXX - doesn't handle domain-literals properly
	 * (but no one uses them)
	 */
	private static void checkAddress(String addr, boolean routeAddr,
			boolean validate) throws AddressException {
		int i, start = 0;
		if (addr.indexOf('"') >= 0)
		...
		if ((i = addr.indexOf('@', start)) >= 0) {
			 ...
			 * separate objects; it writes them out as headers and then parses
			 * the headers when the addresses are requested. In order to support
			 * the case where a "simple" address is used, but the address also
			 * has a personal name and thus looks like it should be a valid
			 * RFC822 address when parsed, we only check this if we're
			 * explicitly called from the validate method.
			 */
			if (validate)
				throw new AddressException("Missing final '@domain'", addr);

			/*
			 * No '@' so it's notreally an RFC822 address, but still we allow
			 * some simple forms.
			 */
			local = addr;
			domain = null;
	...
	/**
	 * Is this a "simple" address? Simple addresses don't contain quotes or any
	 * RFC822 special characters other than '@' and '.'.
	 */
	 ...
	 * group address is different than the mailing list addresses supported by
	...
	/*
	 * public static void main(String argv[]) throws Exception { for (int i = 0;
	 * i < argv.length; i++) { InternetAddress[] a =
	 * InternetAddress.parse(argv[i]); for (int j = 0; j < a.length; j++) {
	 * System.out.println("arg " + i + " address " + j + ": " + a[j]);
	 * System.out.println("\tAddress: " + a[j].getAddress() + "\tPersonal: " +
	 * a[j].getPersonal()); } if (a.length > 1) {
	 * System.out.println("address 0 hash code: " + a[0].hashCode());
	 * System.out.println("address 1 hash code: " + a[1].hashCode()); if
	 * (a[0].hashCode() == a[1].hashCode())
	 * System.out.println("success, hashcodes equal"); else
	 * System.out.println("fail, hashcodes not equal"); if (a[0].equals(a[1]))
	 * System.out.println("success, addresses equal"); else
	 * System.out.println("fail, addresses not equal"); if (a[1].equals(a[0]))
	 * System.out.println("success, addresses equal"); else
